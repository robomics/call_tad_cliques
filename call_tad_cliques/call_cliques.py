...
def __parse_gtrack_record(line: str, header: tuple) -> tuple:
...
def __read_gtrack_file(path_to_gtrack_file: Union[pathlib.Path, str]) -> tuple:
...
def __build_tad_graph(beads: list, interactions: set, chrom: str):
...
def __compute_clique_stats(cliques):
    records = []
    for i, clique in enumerate(cliques):
        records.append([clique[0].getChrom(True), i, len(clique), "CLICSTAT " + str(clique).strip("[]")])

    return pd.DataFrame(records, columns=["chrom", "bead1_id", "bead2_id", "comment"])


def __map_clique_interactions(cliques, clique_size_thresh: int):
    ...
    clique_interactions = [list(n1.getCoords()) + list(n2.getCoords()) for n1, n2 in clique_interactions]
    return pd.DataFrame(clique_interactions, columns=["chrom1", "start1", "end1",
                                                      "chrom2", "start2", "end2"])


def __map_tad_interactions(interactions: set, clique_interactions: set, chrom: str):
    records = []
    ...
    return pd.DataFrame(records, columns=["bead1_id", "bead2_id", "bead_part_of_clique", "comment"])


def __compute_clique_sizes(tad_graph):
    cliquenum = networkx.node_clique_number(tad_graph)
    records = [list(tad.getCoords(True)) + [cliquenum[tad]] for tad in cliquenum]

    return pd.DataFrame(records, columns=["chrom", "start", "end", "size"])


def __preprocess_data(segments: pd.DataFrame, interactions: pd.DataFrame):
    segment_dict = {}
    ...
    data = pd.DataFrame(records, columns=["chrom", "start", "end", "id", "radius", "edges"])
...
def call_cliques(segments: pd.DataFrame, interactions: pd.DataFrame, chrom: str, clique_size_thresh: int):
    ...
    cliques = networkx.find_cliques(tad_graph)

    clique_stats = __compute_clique_stats(cliques)
    clique_interactions = __map_clique_interactions(cliques, clique_size_thresh)
    tad_interactions = __map_tad_interactions(interactions, clique_interactions, chrom)
    clique_sizes = __compute_clique_sizes(tad_graph)

    return clique_stats, clique_interactions, tad_interactions, clique_sizes
